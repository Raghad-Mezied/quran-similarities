...
// Bump this whenever the document layout changes — it's printed in each .docx
// (under the title) so you can tell a freshly-downloaded file from an old one.
...
// Serve the web app (public/index.html) and the fetched JSON data
...
// ---- Generate a real .docx for one surah ----
async function buildDocx(num) {
  const file = path.join(__dirname, "data", `${num}.json`);
  ...
  return { title, buffer: await Packer.toBuffer(doc) };
}

app.get("/word/:num", async (req, res) => {
  const num = parseInt(req.params.num, 10);
  if (!(num >= 1 && num <= 114))
    return res.status(400).send("invalid surah number");
  try {
    const { title, buffer } = await buildDocx(num);
    const ascii = `surah-${num}.docx`;
    // Never cache — always serve a freshly built document.
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${ascii}"; filename*=UTF-8''${encodeURIComponent(title + ".docx")}`,
    );
    res.send(buffer);
  } catch (err) {
    console.error(err);
    res.status(500).send("failed to build document: " + err.message);
  }
});

app.listen(3000, () => {
  console.log("Surah similarities app running at http://localhost:3000");
});
